...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;font-family:'Segoe UI',Tahoma,sans-serif}
    ...
    .container{max-width:780px;width:100%;background:#fff;border-radius:40px;padding:28px 24px 35px;box-shadow:0 20px 50px rgba(0,0,0,0.06)}
    ...
    .container{background:var(--bg);color:var(--text);border:1px solid var(--border)}
    .status-bar{background:var(--primary-light);border-color:var(--border)}
    .member-list{background:var(--card-bg);border-color:var(--border)}
    .member-item{border-bottom-color:var(--border)}
    .status-badge{background:var(--primary-light);color:var(--primary)}
    .status-badge.absent{background:#f1dcdc;color:#8f3b3b}
    .btn-primary{background:var(--primary);color:#fff}
    .btn-primary:hover{filter:brightness(0.85)}
    .admin-panel{background:var(--primary-light);border-color:var(--border)}
    .log-area{background:var(--card-bg);border-color:var(--border)}
    .date-box{background:var(--primary-light);border-color:var(--border)}
    ...
    .loading-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:#0b1a26;display:flex;flex-direction:column;justify-content:center;align-items:center;z-index:9999;transition:opacity 0.7s,visibility 0.7s;opacity:1;visibility:visible}
    .loading-overlay.hidden{opacity:0;visibility:hidden;pointer-events:none}
    .loading-overlay h2{color:#f0e6d3;font-size:32px;font-weight:400;letter-spacing:3px;margin-top:30px;border-top:2px solid #b8946a;padding-top:20px;text-shadow:0 0 12px rgba(255,215,150,0.2)}
    .loader-ring{width:70px;height:70px;border:6px solid #2f4a5c;border-top:6px solid #dbb48b;border-radius:50%;animation:spin 1s linear infinite}
    ...
    .header h1{font-size:24px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:10px}
    ...
    .date-box i{color:var(--accent);cursor:pointer;transition:0.2s;padding:4px}
    .date-box i:hover{color:var(--primary);transform:scale(1.1)}
    .date-box .date-input-group{display:flex;gap:4px;align-items:center}
    .date-box .date-input-group select{padding:4px 8px;border-radius:30px;border:1px solid var(--border);background:var(--bg);font-size:14px;outline:none;cursor:pointer;color:var(--text)}
    .date-box .date-input-group select:focus{border-color:var(--accent)}
    .date-nav-btn{background:none;border:none;font-size:18px;cursor:pointer;color:var(--accent);padding:4px 8px;border-radius:30px;transition:0.2s}
    .date-nav-btn:hover{background:var(--primary-light);color:var(--primary)}
    ...
    .member-name .edit-icon{color:var(--accent);cursor:pointer;font-size:15px}
    ...
    .delay-badge{background:var(--delay-bg);color:var(--delay-color);padding:2px 12px;border-radius:30px;font-size:13px;font-weight:600;display:inline-flex;align-items:center;gap:5px;border:1px solid var(--delay-border);cursor:default;min-width:70px;justify-content:center}
    .delay-badge .delay-input{width:55px;padding:2px 4px;border-radius:20px;border:1px solid var(--delay-border);background:#fff8f0;font-size:13px;text-align:center;outline:none;display:none}
    .delay-badge .delay-input:focus{border-color:var(--primary)}
    .delay-badge .delay-value{cursor:default}
    .delay-badge .delay-edit{cursor:pointer;font-size:12px;color:var(--delay-color);margin-right:2px}
    ...
    .btn{border:none;background:#e4ecf5;padding:12px 22px;border-radius:60px;font-weight:600;font-size:16px;color:var(--text);cursor:pointer;transition:0.2s;display:flex;align-items:center;gap:8px;flex:1 0 auto;justify-content:center;box-shadow:0 2px 6px rgba(0,0,0,0.02)}
    ...
    .admin-row input{flex:1;padding:12px 16px;border-radius:60px;border:1px solid var(--border);background:#fff;font-size:15px;outline:none;transition:0.2s;min-width:120px}
    .admin-row input:focus{border-color:var(--accent)}
    .admin-row .btn{flex:0 1 auto;padding:10px 22px}
    
    .rank-section{margin-top:20px;padding:16px;border-radius:32px;background:var(--rank-bg);border:1px solid var(--border);display:none}
    .rank-section.active{display:block}
    .rank-section h3{font-size:18px;margin-bottom:12px;display:flex;align-items:center;gap:8px;color:var(--text)}
    .rank-section h3 i{color:#f5b342}
    .rank-list{max-height:250px;overflow-y:auto}
    .rank-item{display:flex;align-items:center;justify-content:space-between;padding:8px 12px;border-bottom:1px solid var(--border);font-size:15px}
    .rank-item:last-child{border-bottom:none}
    .rank-item .rank-number{font-weight:700;color:var(--primary);min-width:30px}
    .rank-item .rank-name{flex:1;margin:0 10px;font-weight:500}
    .rank-item .rank-count{background:var(--primary-light);padding:2px 14px;border-radius:30px;font-weight:600;font-size:14px;color:var(--primary)}
    
    .rank-visibility-toggle{display:flex;align-items:center;gap:10px;margin-top:8px;padding:8px 12px;background:var(--primary-light);border-radius:40px;font-size:14px}
    .rank-visibility-toggle .toggle-label{color:var(--text);font-weight:500}
    .rank-visibility-toggle .toggle-switch{position:relative;width:44px;height:24px;background:#b0c4d8;border-radius:30px;cursor:pointer;transition:0.3s;flex-shrink:0}
    .rank-visibility-toggle .toggle-switch.active{background:var(--primary)}
    .rank-visibility-toggle .toggle-switch .toggle-knob{position:absolute;top:2px;left:2px;width:20px;height:20px;background:#fff;border-radius:50%;transition:0.3s;box-shadow:0 2px 6px rgba(0,0,0,0.15)}
    .rank-visibility-toggle .toggle-switch.active .toggle-knob{left:22px}
    .rank-visibility-toggle .toggle-status{font-size:13px;font-weight:600;color:var(--text);min-width:50px}
    ...
    .password-box .btn-secondary{background:#e4ecf5;color:var(--text)}
    ...
    .log-entry{display:flex;justify-content:space-between;padding:8px 4px;border-bottom:1px solid var(--border);font-size:14px}
    ...
    @media (max-width:480px){.container{padding:20px 14px 25px}.header h1{font-size:20px}.clock{font-size:18px}.btn{font-size:14px;padding:10px 14px}.member-item{flex-direction:column;align-items:stretch}.member-status{justify-content:space-between}.date-box{flex-direction:column;align-items:center}.date-box .date-input-group select{font-size:12px;padding:2px 6px}}
  </style>
</head>
...
<script>
(function(){
  // ===== ذخیره و بازیابی اطلاعات از LocalStorage =====
  function loadData(){
    try{
      const saved = localStorage.getItem('attendanceData');
      if(saved){
        const data = JSON.parse(saved);
        if(data.members) masterMembers = data.members;
        if(data.records) records = data.records;
        if(data.logs) logs = data.logs;
        if(data.adminPassword) adminPassword = data.adminPassword;
        if(data.currentDateKey) currentDateKey = data.currentDateKey;
      }
    } catch(e){}
  }
  ...
  function gregorianToPersian(date){
    ...
    return { year: d.getFullYear() - 621, month: d.getMonth() + 1, day: d.getDate() };
  }
  ...
  function updateDayOptions(selectedDay){
    ...
    if(selectedDay > daysInMonth) daySelect.value = daysInMonth;
  }
  ...
  function updateDateFromSelectors(){
    ...
    if(day > maxDay){ day = maxDay; daySelect.value = day; }
  ...
  function updateRanking(){
    const rankMap = {};
    masterMembers.forEach(m => { rankMap[m.id] = { name: m.name, count: 0 }; });
    Object.keys(records).forEach(dateKey => {
      const dayRecords = records[dateKey];
      masterMembers.forEach(m => {
        if(dayRecords[m.id] && dayRecords[m.id].status === 'present') rankMap[m.id].count++;
      });
    });
    ...
    if(sorted.length === 0 || sorted.every(s => s.count === 0)){
      ...
      sorted.forEach((item, index) => {
        ...
        html += `<div class="rank-item"><span class="rank-number">${medal}</span><span class="rank-name">${item.name}</span><span class="rank-count">${item.count} روز</span></div>`;
      });
    }
  ...
  function getRecordForDate(dateKey){
    if(!records[dateKey]){
      records[dateKey] = {};
      masterMembers.forEach(m => { records[dateKey][m.id] = { status: 'present', delay: 0 }; });
    }
  ...
  function attachMemberEvents(){
    ...
    document.querySelectorAll('.member-remove').forEach(el => {
      el.addEventListener('click', function(e){
        ...
        Object.keys(records).forEach(key => { delete records[key][id]; });
        renderMembers();
        updateStatusBar();
        updateRanking();
        saveData();
  ...
  function updateStatusBar(){
    const rec = getCurrentRecord();
    const present = masterMembers.filter(m => { const data = rec[m.id]; return data && data.status === 'present'; }).length;
    const total = masterMembers.length;
    const hasDelay = masterMembers.some(m => { const data = rec[m.id]; return data && data.delay > 0; });
    if(total === 0){ statusDot.className = 'status-dot'; statusText.textContent = 'هیچ عضوی ثبت نشده'; return; }
    const ratio = present / total;
    let delayText = hasDelay ? ' ⚠️ تاخیر دارد' : '';
    if(ratio >= 0.6){ statusDot.className = 'status-dot'; statusText.textContent = `${present} نفر حاضر${delayText}`; }
    else if(ratio > 0.2){ statusDot.className = 'status-dot warning'; statusText.textContent = `${present} نفر حاضر${delayText}`; }
    else { statusDot.className = 'status-dot inactive'; statusText.textContent = `${present} نفر حاضر${delayText}`; }
  }

  function renderLogs(){
    if(logs.length === 0){ logContainer.innerHTML = `<div class="text-muted" style="padding:16px 0; text-align:center;">ثبتی وجود ندارد</div>`; return; }
  ...
  function addLog(type, name){
    ...
    renderLogs();
    saveData();
    if(type === 'in'){ statusDot.className = 'status-dot'; statusText.textContent = `${name} وارد شد`; }
    else if(type === 'out'){ statusDot.className = 'status-dot inactive'; statusText.textContent = `${name} خارج شد`; }
    else if(type === 'delay'){ statusText.textContent = `⏱ ${name}`; }
    else if(type === 'saveDay'){ statusText.textContent = `✅ روز ${currentDateKey} ثبت شد`; }
    else if(type === 'password'){ statusText.textContent = `🔑 رمز عبور تغییر کرد`; }
    else { statusText.textContent = `گزارش`; }
  ...
  saveDayBtn.addEventListener('click', function(){
    if(!isAdmin) return;
    const rec = getCurrentRecord();
    const present = masterMembers.filter(m => { const data = rec[m.id]; return data && data.status === 'present'; }).map(m => m.name).join('، ') || 'هیچ';
    const absent = masterMembers.filter(m => { const data = rec[m.id]; return data && data.status === 'absent'; }).map(m => m.name).join('، ') || 'هیچ';
    const delays = masterMembers.filter(m => { const data = rec[m.id]; return data && data.delay > 0; }).map(m => `${m.name} (${rec[m.id].delay} دقیقه)`).join('، ') || 'بدون تاخیر';
  ...
  resetAllBtn.addEventListener('click', function(){
    ...
    const rec = getCurrentRecord();
    masterMembers.forEach(m => { if(!rec[m.id]) rec[m.id] = { status: 'present', delay: 0 }; rec[m.id].status = 'present'; });
    renderMembers();
    updateStatusBar();
    updateRanking();
    saveData();
  ...
  savePasswordBtn.addEventListener('click', function(){
    ...
    if(newPass.length < 4){ alert('رمز عبور باید حداقل ۴ کاراکتر باشد'); return; }
  ...
  cancelPasswordBtn.addEventListener('click', function(){ passwordModal.classList.remove('show'); });
  passwordModal.addEventListener('click', function(e){ if(e.target === this) this.classList.remove('show'); });
  newPassword.addEventListener('keydown', function(e){ if(e.key === 'Enter') confirmPassword.focus(); });
  ...
  toggleRankBtn.addEventListener('click', function(){
    ...
    if(showRank){ updateRanking(); this.innerHTML = '<i class="fas fa-trophy"></i> بستن رتبه‌بندی'; }
    else { this.innerHTML = '<i class="fas fa-trophy"></i> رتبه‌بندی'; }
  });
  ...
  rankSwitch.addEventListener('click', function(){
    ...
    if(!showRank){ showRank = true; rankSection.classList.add('active'); toggleRankBtn.innerHTML = '<i class="fas fa-trophy"></i> بستن رتبه‌بندی'; updateRanking(); }
    if(rankVisibleForMembers){ alert('✅ رتبه‌بندی برای اعضا قابل مشاهده شد'); }
    else { alert('❌ رتبه‌بندی برای اعضا مخفی شد'); }
  });
  ...
  document.getElementById('datePrevMonth').addEventListener('click', function(){
    const p = gregorianToPersian(currentDate);
    let newMonth = p.month - 1, newYear = p.year;
  ...
  yearSelect.addEventListener('change', function(){ const currentDay = parseInt(daySelect.value) || 1; updateDayOptions(currentDay); updateDateFromSelectors(); });
  monthSelect.addEventListener('change', function(){ const currentDay = parseInt(daySelect.value) || 1; updateDayOptions(currentDay); updateDateFromSelectors(); });
  daySelect.addEventListener('change', function(){ updateDateFromSelectors(); });
  ...
  adminPass.addEventListener('keydown', function(e){ if(e.key === 'Enter') secretLoginBtn.click(); });
  secretModal.addEventListener('click', function(e){ if(e.target === this) this.classList.remove('show'); });
